...
class SpecifyFollowRequestStatusAPITest(APITestCase):
    ...
    def test_accept_follow_relationship_when_not_exists(self):
        ...
        response = self.client.post('/api/user/100/request-status/', HTTP_AUTHORIZATION=token)
    ...
    def test_accept_follow_request(self):
        ...
        user = get_user_model().objects.get(id=4)
        token = f'Bearer {str(AccessToken.for_user(user))}'
        response = self.client.post('/api/user/2/request-status/', HTTP_AUTHORIZATION=token)
    ...
    def test_reject_follow_request_when_not_exists(self):
        user = get_user_model().objects.get(id=4)
        token = f'Bearer {str(AccessToken.for_user(user))}'
        response = self.client.delete('/api/user/100/request-status/', HTTP_AUTHORIZATION=token)
        self.assertEqual(status.HTTP_404_NOT_FOUND, response.status_code)
    ...
    def test_reject_follow_request(self):
        ...
        user = get_user_model().objects.get(id=4)
        token = f'Bearer {str(AccessToken.for_user(user))}'
        response = self.client.delete('/api/user/3/request-status/', HTTP_AUTHORIZATION=token)
